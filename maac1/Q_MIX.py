...
from common.replay_buffer_qmix import ReplayBuffer
...
class QMIX_Agent():
    ...
    def save_memory(self, obs_and_u_last, state, \
        ...
        state_new = np.array([state_new])[np.newaxis, :]
    ...
    def cal_totq_values(self, batch_data, args):
        ...
        print('len(new_avail_act_n)',len(new_avail_act_n))
        for i in range(len(new_avail_act_n)):
            for j in range(2):
                for m in range(5):
                    if new_avail_act_n[i][0][j][m]==0.0:
                        new_avail_act_n[i][0][j][m] = 0
                    else:
                        new_avail_act_n[i][0][j][m]=1
