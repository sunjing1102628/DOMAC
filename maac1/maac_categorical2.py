import torch
import os
#from maac1.actor_critic_categorical2 import Actor, Critic
from maac1.actor_critic_categorical2 import Actor, TwinnedQNetwork
import numpy as np
...
class MAAC:
    def __init__(self, args,seed):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        self.critic = TwinnedQNetwork(args.n_agents, args.state_dim, args.action_dim, seed).to(device)
        # print('self.critic',self.critic)

        self.critic_target = TwinnedQNetwork(args.n_agents,  args.state_dim, args.action_dim,seed).to(device)
    ...
    def train(self):
        ...
        for i in range(self.agent_num):
            ...
            Q_target1,Q_target2 = self.critic_target(input_critic)
            Q_target = torch.min(Q_target1, Q_target2).detach()
            print('Q_target is',Q_target)
            # print('Q_target_size is', Q_target.size())
            ...
            Q1,Q2 = self.critic(input_critic)
            Q= torch.min(Q1,Q2)
            # print('Q is',Q.size())
    ...
    def build_input_critic(self, agent_id, observations,actions):
        ...
        # action = [torch.nn.functional.one_hot(action[0].to(torch.int64), 5),           torch.nn.functional.one_hot(
        #     action[1].to(torch.int64), 5),           torch.nn.functional.one_hot(action[2].to(torch.int64), 5)] 
        ...
        #input_critic = torch.cat([ids.to(device), observations.type(torch.float32).to(device)], dim=-1)
