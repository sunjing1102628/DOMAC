...
from maac1.actor_critic_maac_QR import Actor, Critic
import numpy as np
...
class DPPO:
    ...
    def save_model_best(self):
        ...
        for agent_id, actor_net in enumerate(self.actors):
            model_path = os.path.join(self.args.save_dir7b, self.args.algorithm_name3)
    ...
    def save_model(self, train_step):  # old save fn
        ...
        for agent_id, actor_net in enumerate(self.actors):
            model_path = os.path.join(self.args.save_dir7a, self.args.algorithm_name3)
            if not os.path.exists(model_path):
                os.makedirs(model_path)
            model_path = os.path.join(model_path, 'agent_%d' % agent_id)
            if not os.path.exists(model_path):
