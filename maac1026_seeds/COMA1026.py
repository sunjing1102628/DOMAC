...
class COMA:
    def __init__(self, agent_num, state_dim, action_dim, lr_c, lr_a, gamma, target_update_steps,seed):
        self.agent_num = agent_num
       # print('self.agent_num',self.agent_num)
        ...
        self.actors = [Actor(state_dim, action_dim,seed).to(device) for _ in range(agent_num)]
        #print('self.actors',self.actors)
        self.critic = Critic(agent_num, state_dim, action_dim,seed).to(device)
    ...
    def train(self):
        ...
        for i in range(self.agent_num):
            ...
            Q = self.critic(input_critic).to(device)
            #print('Q is',Q)
